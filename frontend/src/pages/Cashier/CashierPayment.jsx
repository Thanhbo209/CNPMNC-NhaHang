      ...
      const html = `
        ...
        <body>
          <div class="header">
            ...
            <div>Order: ${order._id || '-'} | Bàn: ${order.table?.tableNumber || order.tableNumber || '-'}</div>
          ...
          <table>
            ...
            <tbody>
              ${items.map(it => {
                ...
                return `<tr><td>${name}</td><td class="right">${qty}</td><td class="right">${price.toLocaleString('vi-VN')}₫</td><td class="right">${line.toLocaleString('vi-VN')}₫</td></tr>`;
              }).join('')}
            </tbody>
          </table>
          <div style="margin-top:12px; text-align:right">
            <div>Thành tiền: ${p.subtotal?.toLocaleString('vi-VN') || 0}₫</div>
            <div>Thuế (${p.taxPercent || 8}%): ${p.taxAmount?.toLocaleString('vi-VN') || 0}₫</div>
            <div style="font-weight:bold; font-size:1.1em">Tổng: ${p.amount?.toLocaleString('vi-VN') || 0}₫</div>
                ...
                <div className="mb-3">
                    ...
                    {(order.items || []).map((item, i) => (
                        ...
                        <span className="text-green-600">
                            {(item.food?.price * item.quantity || 0).toLocaleString()}₫
                        </span>
                        </p>
                    ))}
                    {(order.addedItems || []).map((item, i) => (
                        ...
                        >
                        <span>
                            {item.food?.name || "Món đã xóa"} x {item.quantity} (món thêm)
                        </span>
                        <span className="text-green-600">
                            {(item.food?.price * item.quantity || 0).toLocaleString()}₫
                        </span>
      ...
      <div className="mt-6">
        <div className="flex items-center justify-between p-3 border-t bg-white">
          <div className="text-sm text-gray-600">
            Hiển thị <span className="font-semibold">{Math.min((page - 1) * limit + 1, total || 0)}</span> - <span className="font-semibold">{Math.min(page * limit, total || 0)}</span> trên <span className="font-semibold">{total || 0}</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: pages }).slice(Math.max(0, page - 3), Math.min(pages, page + 2)).map((_, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-sm ${pageNum === page ? 'bg-gray-900 text-white' : 'bg-white border'}`}
                  >
                    {pageNum}
                  </button>
                );
